...
# 1. Define the font filename (Must be in the repository root alongside app.py)
...
# THIS FUNCTION IS UPDATED TO INCLUDE TEXT WRAPPING AND PROPORTIONAL SIZING
def add_caption(image, caption, font_path=FONT_PATH):
    ...
    # Define font size based on image height for proportionality (e.g., 10% of image height)
    ...
    try:
        ...
        # If the custom font fails, the warning is shown, and the default font is used
    ...
    def get_text_dimensions(text, current_font):
        # We use a try/except because textbbox is better but not supported by all PIL versions/fonts
        try:
            # spacing=0 is crucial to prevent squishing with the default font
    ...
    for adj_x in range(-stroke_width, stroke_width + 1):
        for adj_y in range(-stroke_width, stroke_width + 1):
            ...
            # Use align="center" for multi-line text
